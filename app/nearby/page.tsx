...
type Place = {
  ...
  types: string[]; category: string; typeLabel: string; photo?: string
}
...
export default function Nearby() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setError('請允許位置權限，AI 先可以搵附近題材'); setLoading(false) },
  ...
  return (
    <main style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border)' }}>
        ...
        {(loading || !coords) && <div style={{ width: 60 }} />}
      </div>
      ...
      <div style={{ flex: 1, padding: '16px 20px 40px', maxWidth: 480, width: '100%', margin: '0 auto' }}>
        ...
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, paddingTop: 80 }}>
            ...
            <p style={{ color: 'var(--ink3)', fontSize: 13 }}>搵緊附近 5 類地點…</p>
        ...
        {!loading && !error && places.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <p style={{ fontSize: 12, color: 'var(--ink3)', marginBottom: 6 }}>
              附近 {places.length} 個推薦地點（餐廳 · 景點 · 咖啡 · 博物館 · 商店）
            </p>
            {places.map(p => (
              <button key={p.id} onClick={() => selectPlace(p)} style={{ width: '100%', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 0, textAlign: 'left', cursor: 'pointer', overflow: 'hidden', transition: 'border-color 0.15s' }}>
                {p.photo && (
                ...
                <div style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--accent2)', background: 'rgba(77,107,254,0.12)', padding: '2px 8px', borderRadius: 'var(--pill)', flexShrink: 0 }}>{p.typeLabel}</span>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--ink3)', marginBottom: p.rating ? 4 : 0 }}>{p.address}</div>
                  {p.rating > 0 && <div style={{ fontSize: 12, color: '#FAC775' }}>★ {p.rating.toFixed(1)}</div>}
                </div>
              </button>
            ))}
          </div>
